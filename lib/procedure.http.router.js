 ...
 * is applicable.
 * @apiSuccess {Activity} activity Activity under which a procedure is applicable.
 ...
 * @apiSuccess {Procedure[]} data List of procedures
 ...
 * is applicable.
 * @apiSuccess {Activity} data.activity Activity under which a procedure is applicable.
...
/* declarations */
const Procedure = include(__dirname, 'procedure.model');
...
router.get(PATH_LIST, function getProcedures(request, response, next) {
  ...
  Procedure.get(options, function onGetProcedures(error, results) {
    //forward error
    if (error) {
      next(error);
    }
...
router.post(PATH_LIST, function postProcedure(request, response, next) {
  ...
  Procedure.post(body, function onPostProcedure(error, created) {
    //forward error
    if (error) {
      next(error);
    }
...
router.get(PATH_SINGLE, function getProcedure(request, response, next) {
  ...
  Procedure.getById(options, function onGetProcedure(error, found) {
    //forward error
    if (error) {
      next(error);
    }
...
router.patch(PATH_SINGLE, function patchProcedure(request, response, next) {
  ...
  Procedure.patch(id, patches, function onPatchProcedure(error, patched) {
    //forward error
    if (error) {
      next(error);
    }
...
router.put(PATH_SINGLE, function putProcedure(request, response, next) {
  ...
  Procedure.put(id, updates, function onPutProcedure(error, updated) {
    //forward error
    if (error) {
      next(error);
    }
...
router.delete(PATH_SINGLE, function deleteProcedure(request, response, next) {
  ...
  Procedure.del(id, function onDeleteProcedure(error, deleted) {
    //forward error
    if (error) {
      next(error);
    }
