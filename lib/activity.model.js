...
const ACTIVITY_COLLECTION_NAME = getString(
  'ACTIVITY_COLLECTION_NAME',
  'activities'
);
const ACTIVITY_SEED = getString('ACTIVITY_SEED', 'activities');
...
const ActivitySchema = new Schema(
  {
    ...
    /**
     * @name number
     * @description Acitvity number(or sequence) for ordering in relation to
...
ActivitySchema.methods.preValidate = function preValidate(done) {
  ...
  this.incidentType = this.incidentType || _.get(this, 'plan.incidentType');

  //ensure disaster phase
  this.phase = this.phase || DEFAULT_DISASTER_PHASE;
 ...
 * @param {activity} activity valid activity details
 * @param {function} done callback to invoke on success or error
 *
 * @author lally elias <lallyelias87@gmail.com>
 * @since 0.1.0
 ...
 * @public
 */
ActivitySchema.statics.upsert = function upsert(activity, done) {
  //normalize arguments
  const _activity = _.isFunction(activity.toObject)
    ? activity.toObject()
    : activity;
  ...
  async.waterfall(
    [
      function findExistingActivity(next) {
        let criteria = _.merge({}, _activity);
        criteria = criteria._id
          ? _.pick(criteria, '_id')
          : _.pick(criteria, 'phase', 'name');
        Activity.findOne(criteria, next);
      },

      function upsertActivity(found, next) {
        //instantiate if not found
        if (!found) {
          found = new Activity(_activity);
        }

        //upsert
        found.updatedAt = new Date();
        found.put(_activity, next);
      },
    ],
    done
  );
 ...
 * @name seed
 * @function seed
 * @description seed activities into database. On duplicate last changes will
 * override existing one.
 * @param {Role[]} [activities] set of permission(s) to seed
 * @param {function} done callback to invoke on success or error
 *
 * @author lally elias <lallyelias87@gmail.com>
 * @since 0.1.0
 ...
 * @public
 */
ActivitySchema.statics.seed = function seed(seeds, done) {
  //normalize arguments
  ...
  const SEED_PATH = path.join(SEEDS_PATH, ACTIVITY_SEED);
  try {
  ...
  // collect unique activities from seeds
  _seeds = _.compact(_seeds);
  _seeds = _.uniqWith(_seeds, _.isEqual);

  //upsert activities
  let activities = [].concat(_seeds);
  activities = _.map(activities, function(activity) {
    return function upsertActivities(next) {
      Activity.upsert(activity, next);
    };
  });

  //seed activities
  async.parallel(activities, _done);
};
